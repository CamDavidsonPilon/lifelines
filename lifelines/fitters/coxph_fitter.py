        ...
        while converging:
            ...
            if self.strata is None:
                ...
                for strata in np.unique(X.index):
                    stratified_X, stratified_T, stratified_E, stratified_W = (
                        X.loc[[strata]],
                        T.loc[[strata]],
                        E.loc[[strata]],
                        weights.loc[[strata]],
    ...
    def _compute_sandwich_estimator(self, X, T, E, weights):
        ...
        if self.strata is not None:
            ...
            for strata in np.unique(X.index):
                ...
                stratified_X, stratified_E, stratified_W = (
                    X.loc[[strata]],
                    E.loc[[strata]],
                    weights.loc[[strata]],
                )
        ...
        if self.cluster_col:
            ...
            for cluster in np.unique(self._clusters):
                ...
                score_residuals_ = np.append(
                    score_residuals_, (score_residuals[ix, :] * weights_[:, None]).sum(0).reshape(1, d), axis=0
                )
