...
class AalenJohansenFitter(UnivariateFitter):
    ...
    def fit(self,durations, event_observed, event_indicator, timeline=None, entry=None, label='AJ_estimate', 
        ...
        if np.sum(durations.duplicated()) > 0:
            ...
            durations = self._jitter(durations=durations, event=event_observed, jitter_level=self._jitter_level)
        ...
        km = KaplanMeierFitter()  # TODO this uses as exploit in that KM does not check that event indicator is 0/1
